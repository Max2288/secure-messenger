...
class MessageReadRepository:
    ...
    async def delete_for_user(self, message_id: int, user_id: int) -> None:
        stmt = delete(MessageRead).where(MessageRead.message_id == message_id, MessageRead.user_id == user_id)
    ...
    async def mark_many_as_read(self, message_ids: list[int], user_id: int) -> None:
        existing_stmt = select(MessageRead.message_id).where(
            MessageRead.user_id == user_id, MessageRead.message_id.in_(message_ids)
        )
        ...
        if new_message_ids:
            self.session.add_all([MessageRead(user_id=user_id, message_id=mid) for mid in new_message_ids])
            await self.session.commit()
    ...
    async def get_read_message_ids(self, user_id: list[int], message_ids: list[int]) -> set[int]:
        ...
        stmt = select(MessageRead.message_id).where(MessageRead.user_id.in_(user_id), MessageRead.message_id.in_(message_ids))
        result = await self.session.execute(stmt)
